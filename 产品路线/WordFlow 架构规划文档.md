...
**设计理念：**
- **基础功能完全免费**：热词、快捷短语作为软件基础功能，不收费
- **内容付费**：专业词典包由创作者制作，用户购买
- **平台分成**：创作者 70%，平台 30%
...
│  ┌─────────────────┐    ┌─────────────────┐            │
│  │   Vocabulary    │    │   Shortcut      │            │
│  │   HotwordMgr    │    │   PhraseMgr     │            │
│  │   TermLearner   │    │   TriggerMatch  │            │
│  └─────────────────┘    └─────────────────┘            │
│           ↓                      ↓                      │
│  ┌─────────────────────────────────────────┐           │
...
**作用**：提高语音识别对特定词汇的敏感度，给 AI 加权更容易识别出这些词

**数据结构**：
```json
{
  "hotwords": [
    { "word": "室上性心动过速", "weight": 10, "category": "医疗" },
    { "word": "冠状动脉", "weight": 8, "category": "医疗" },
    { "word": "WordFlow", "weight": 10, "category": "通用" }
  ]
}
```
...
stream = recognizer.create_stream(
    hotwords={"室上性心动过速": 10, "冠状动脉": 8}
)
```

#### 4.2.2 快捷短语功能 (Shortcuts)

**作用**：语音说出触发词，展开成一大段预设文本
...
public class HotwordLearner
{
    ...
    public Dictionary<string, int> GenerateHotwords()
    {
        return _wordFreq
            .Where(kvp => kvp.Value >= 10)
            .ToDictionary(kvp => kvp.Key, kvp => Math.Min(kvp.Value / 5, 10));
    }
}
```
...
**安装流程**：
```
选择词典包文件 → 验证格式 → 检查购买 → 解压安装 → 应用热词
```

**词库分类：**
| 分类 | 说明 | 示例 | 来源 |
|------|------|------|------|
| 通用词库 | 基础词汇 | 日常用语、常用词 | 官方免费 |
| 专业词库 | 行业术语 | 医疗、法律、编程 | 创作者/官方 |
| 人名库 | 人名、地名 | 客户名、供应商 | 用户自制 |
| 自定义词库 | 用户创建 | 个人专有词汇 | 用户自制 |
...
### 10.1 决策:云端 API 集成策略 (2026-03-02)

#### 10.1.1 决策背景

随着 AI 技术发展,Typeless 等竞品通过云端大模型实现了全流程智能优化,WordFlow 面临技术路线选择:是否接入云端 API 以提升产品竞争力?

#### 10.1.2 方案对比

| 维度 | 方案一:云端 ASR | 方案二:本地识别 + 云端优化 |
|------|----------------|------------------------|
| **架构** | 语音数据 → 云端 ASR API → 文本 | 语音数据 → 本地识别 → 云端优化 API → 文本 |
| **离线能力** | ❌ 完全依赖网络 | ✅ 核心流程离线 |
| **隐私保护** | ⚠️ 语音数据上传云端 | ✅ 语音不出本地 |
| **成本** | 高 (ASR 按秒计费,~0.00022 元/秒) | 低 (文本优化成本极低) |
| **延迟** | 取决于网络 | 本地零延迟 + 异步优化 |
| **核心价值** | ❌ 与 Typeless 同质化 | ✅ 差异化竞争 |
...
1. **PM 视角 - 用户体验**
   - 坚守"离线优先"核心价值,保护核心用户(医生/律师/记者)隐私需求
   - 基础功能免费(离线识别),增值服务付费(云端优化),符合 Freemium 模式
   - 即使网络不佳或关闭云端功能,核心体验不受影响

2. **市场视角 - 竞争优势**
   - 定位:既有 Typeless 的"智能",又有 CapsWriter-Offline 的"独立"
   - 避免与 Typeless 正面竞争,形成独特护城河
   - 成本可控,可纳入专业版/企业版订阅费

3. **技术视角 - 可行性**
   - DeepSeek **无官方 ASR API**,但 Chat API 可用于文本润色/纠错
   - 文本优化 Prompt 示例:`请润色以下口语化文本,修正错别字,添加合适标点,使其更符合书面语格式:[文本]`
   - 智谱/阿里云 ASR API 可作为独立的"联网精准识别模式"(可选功能)

#### 10.1.4 智能混合架构设计

```mermaid
flowchart TD
    A[用户语音] --> B[本地 sherpa-onnx 快速识别]
    B --> C[文本 A<br/>即时显示,零延迟]
    C --> D{用户设置}
    D -->|自动优化| E[云端 DeepSeek Chat API]
    D -->|手动触发| F[点击增强按钮]
    F --> E
    E --> G[文本 B<br/>润色纠错结果]
    G --> H[对比显示/替换]
    H --> I[VocabularyLearningEngine<br/>从差异中学习]
    I --> J[反哺本地词库/模型]
```

**技术要点:**

1. **即时反馈**:本地识别文本 A 立即显示,保证零延迟体验
2. **异步优化**:云端优化在后台进行,不阻塞主线程
3. **用户裁决**:提供"原文"与"优化后"对比视图,用户最终确认
4. **持续学习**:从用户选择中学习,优化本地模型

#### 10.1.5 技术风险与缓解

| 风险 | 缓解措施 |
|------|---------|
| 本地识别错误导致优化 API 幻觉 | 提供"原文 vs 优化"对比视图,用户裁决 |
| 网络延迟影响体验 | 异步处理 + 超时机制,网络不佳自动放弃 |
| 成本失控 | 频率限制 + 内容长度检查,防止滥用 |

#### 10.1.6 实施路线

**短期(MVP):**
- [ ] 全力打磨本地离线识别准确率和稳定性(生命线)
- [ ] 设置中预留"实验性功能:云端文本优化(即将推出)"选项
- [ ] 收集用户意向数据

**中期(增值功能):**
- [ ] 申请 DeepSeek API Key,测试文本润色/纠错能力
- [ ] 测试阿里云/智谱 ASR API 作为技术储备
- [ ] 设计非侵入式交互流程(如"魔法棒"图标)
- [ ] 实现异步优化架构

**长期(功能扩展):**
- [ ] 将"联网精准识别模式"作为独立选项(明确隐私提示)
- [ ] 从用户选择中学习,优化本地模型
- [ ] 持续优化成本控制策略

#### 10.1.7 核心结论

> **不要用云端 API 替代本地引擎,而要用它来赋能和增强本地引擎的输出结果。**

WordFlow 的成功在于:坚持"离线优先"根基,采用"本地为主,云端增强"的混合智能架构,成为一款既有 Typeless 的"智能",又有 CapsWriter-Offline 的"独立"的独特产品。
