# WordFlow v1.0

WordFlow 是一款智能语音输入工具，支持本地 ASR 语音识别，可将语音实时转换为文字并输入到任意应用程序中。

**v1.0 正式版** - 基于 .NET 8 WPF 开发，提供稳定、智能的语音输入体验。

---

## 📥 立即下载

### 方式一：安装程序（推荐）

| 下载项 | 文件大小 | 说明 |
|--------|---------|------|
| [⬇️ WordFlow_Setup.exe](https://github.com/yanlin-cheng/WordFlow/releases/download/v1.0.0/WordFlow_Setup.exe) | 约 350 MB | **完整安装包**（内置 .NET 8 运行时和 Python 环境，无需额外安装） |

### 方式二：手动下载模型（可选）

如果自动下载失败，可手动下载模型文件放到 `PythonASR/models/` 目录：

| 下载项 | 文件大小 | 说明 |
|--------|---------|------|
| [🧠 SenseVoice 模型 (1/2)](https://github.com/yanlin-cheng/WordFlow/releases/download/v1.0.0/sensevoice-small-onnx.zip.001) | 90 MB | 语音模型分卷 1 |
| [🧠 SenseVoice 模型 (2/2)](https://github.com/yanlin-cheng/WordFlow/releases/download/v1.0.0/sensevoice-small-onnx.zip.002) | 57 MB | 语音模型分卷 2 |

> **💡 提示**: 推荐使用安装程序安装，首次启动时会自动下载模型。

---

## 🚀 快速开始

### 安装步骤

1. **下载安装包**：下载 `WordFlow_Setup.exe` 安装程序
2. **运行安装程序**：双击安装包，按照提示完成安装
   - 安装程序会自动检测并安装 .NET 8 运行环境（如未安装）
   - 可选择在安装完成后立即启动 WordFlow
3. **启动程序**：安装完成后即可启动使用

### 模型安装

**推荐方式**：首次启动时会自动提示下载模型

**备用方式**：如自动下载失败，可在程序中下载：
1. 启动 WordFlow
2. 点击「模型管理」按钮
3. 在模型管理页面点击模型的「下载」按钮
4. 下载完成后会自动尝试加载模型

### 使用说明

1. **启动 WordFlow** - 程序会最小化到系统托盘
2. **按住说话** - 按住语音输入热键（默认 `CapsLock`）开始录音
3. **松手上屏** - 松开热键自动识别并输入文字到目标应用

---

## ✨ 功能特性

### 核心功能

| 功能 | 说明 |
|------|------|
| 🎤 **智能语音输入** | 按住说话，松手自动识别上屏 |
| 🧠 **本地 ASR 识别** | 使用 Sherpa-ONNX 框架，离线可用 |
| 📚 **个人词库** | 支持自定义词汇，AI 智能纠错 |
| 📝 **历史记录** | 自动保存输入历史，方便查阅 |
| 🔔 **托盘集成** | 最小化到系统托盘，随时调用 |
| ⚡ **开机自启** | 支持设置开机自动启动 |
| ⌨️ **全局热键** | 可自定义热键 |
| 🔄 **自动模型加载** | 下载完成后自动尝试加载模型 |

### 热键设置

WordFlow 支持多种热键配置，满足不同使用习惯：

| 热键类型 | 默认值 | 说明 |
|---------|--------|------|
| 语音输入热键 | CapsLock (大写锁定键) | 按住说话，松手识别 |

> **💡 提示**: 推荐使用大写锁定键（CapsLock）作为语音输入热键，因为：
> - 位置居中，左右手都能方便操作
> - 按键面积大，不易误触
> - 按下时有明显的物理反馈

### 模型管理

WordFlow 提供模型管理功能：
- **一键下载**：点击「下载」按钮直接下载模型，无需重复选择
- **自动加载**：下载完成后自动尝试加载模型
- **服务检测**：如 ASR 服务未启动，会提示是否启动
- **状态显示**：实时显示模型状态（已安装/未下载/已加载）

---

## 💻 系统要求

| 项目 | 要求 |
|------|------|
| 操作系统 | Windows 10/11 64 位 |
| 运行环境 | .NET 8.0 Runtime（安装包已内置） |
| 磁盘空间 | 至少 500MB 可用空间（含模型文件） |
| 网络 | 首次启动需要联网下载模型 |
| 麦克风 | 需要可用的音频输入设备 |
| 内存 | 建议 4GB 以上 |

---

## 🏗️ 技术架构

### 技术栈

| 组件 | 技术 | 说明 |
|------|------|------|
| UI 框架 | .NET 8 WPF | 成熟稳定，Windows 原生支持 |
| 数据库 | SQLite | 轻量级，无需额外服务 |
| ORM | Dapper | 轻量级 ORM，性能好 |
| 语音识别 | Sherpa-ONNX | Next-Gen K2 语音识别工具包 |
| 音频处理 | NAudio | 成熟的 .NET 音频库 |
| 分词引擎 | Jieba.NET | 中文分词 |
| 拼音转换 | TinyPinyin | 汉字转拼音 |
| 日志 | 内置 Logger | 结构化日志 |

### 目录结构

```
WordFlow/
├── WordFlow.exe              # 主程序
├── Data/
│   └── models.json           # 模型配置
├── PythonASR/
│   ├── asr_server.py         # ASR 服务端
│   ├── start_server.bat      # 启动脚本
│   └── models/
│       └── sensevoice-small-onnx/  # 语音模型（首次启动下载）
│           ├── model.int8.onnx
│           └── tokens.txt
├── Resources/
│   └── icon.ico              # 应用图标
└── Logs/                     # 日志文件
```

### 服务架构

```
┌─────────────────────────────────────────────────────────┐
│                    WordFlow 客户端                       │
│  ┌─────────────────────────────────────────────────┐   │
│  │              .NET WPF 界面层                     │   │
│  │  - 主窗口 (MainWindow)                          │   │
│  │  - 设置窗口 (SettingsWindow)                    │   │
│  │  - 词库管理 (VocabularyManagerWindow)           │   │
│  │  - 模型管理 (ModelManagerWindow)                │   │
│  │  - 首次运行向导 (FirstRunWizard)                │   │
│  └─────────────────────────────────────────────────┘   │
│                          ↓                               │
│  ┌─────────────────────────────────────────────────┐   │
│  │              .NET 服务层                         │   │
│  │  - SettingsService (设置管理)                   │   │
│  │  - HistoryService (历史记录/SQLite)             │   │
│  │  - VocabularyService (词库管理)                 │   │
│  │  - VocabularyLearningEngine (词库学习)          │   │
│  │  - GlobalHotkeyService (全局热键)               │   │
│  │  - TrayService (系统托盘)                       │   │
│  │  - ModelDownloadService (模型下载)              │   │
│  │  - SpeechRecognitionService (语音识别客户端)     │   │
│  └─────────────────────────────────────────────────┘   │
│                          ↓ HTTP                        │
│  ┌─────────────────────────────────────────────────┐   │
│  │          Python ASR 服务 (独立进程)              │   │
│  │  - Sherpa-ONNX 语音识别                         │   │
│  │  - 模型加载/切换                                 │   │
│  │  - 音频预处理                                   │   │
│  └─────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────┘
```

---

## ❓ 常见问题

### Q1: 第一次启动需要下载什么？

首次启动会自动下载语音模型文件（约 156MB），请保持网络畅通。下载完成后即可正常使用，后续无需再次下载。

### Q2: 模型下载失败怎么办？

如果自动下载失败，可以：
1. 检查网络连接是否正常
2. 尝试重新启动程序
3. 手动下载模型文件（见上方「方式二」）
4. 在模型管理页面点击「刷新」按钮重试

### Q3: 下载完模型后为什么不能使用？

WordFlow 已优化此问题：
- 下载完成后会自动尝试加载模型
- 如 ASR 服务未启动，会提示是否启动
- 如仍无法使用，请重启程序或手动点击「连接服务」按钮

### Q4: 识别不准确怎么办？

1. 检查麦克风是否正常工作
2. 在安静环境下使用
3. 吐字清晰，语速适中
4. 可以在个人词典中添加常用词汇

### Q5: 某些程序无法输入文字？

某些程序可能限制外部输入，可以尝试：
1. 以管理员身份运行 WordFlow
2. 在目标程序中先点击文本框获得焦点
3. 检查是否有安全软件拦截

### Q6: 如何修改热键？

1. 右键点击系统托盘图标
2. 选择「设置」
3. 在「热键设置」中修改

### Q7: 开机后没有自动启动？

1. 检查是否勾选了「开机自启动」选项
2. 检查安全软件是否拦截了自启动
3. 在任务管理器的「启动」标签中确认

---

## 📋 更新日志

### v1.0.0 (2026-03-15)

**首次发布**
- 🎉 实现基础语音输入功能
- 🎉 实现个人词典管理
- 🎉 实现历史记录功能
- 🎉 实现系统托盘集成
- 🎉 实现开机自启动
- 🎉 实现全局热键

**技术特性**
- 🏗️ 基于 .NET 8 WPF 开发
- 🏗️ 采用模块化服务架构
- 🏗️ 内置 Python 环境和 ASR 服务
- 🏗️ 支持多模型下载和管理

**部署优化**
- 🚀 自包含发布，无需安装 .NET Runtime
- 🚀 安装包约 350MB，包含所有依赖
- 🚀 模型下载移至首次启动，支持断点续传
- 🚀 简化安装流程，提升安装成功率

**已知问题**
- 某些应用程序可能不完全兼容语音输入上屏功能

---

## 🔗 项目链接

| 平台 | 链接 |
|------|------|
| 📂 **GitHub 源码仓库** | https://github.com/yanlin-cheng/WordFlow |
| 🐛 **问题反馈** | https://github.com/yanlin-cheng/WordFlow/issues |

### 多语言版本

- [🇨🇳 中文](README.md)
- [🇬🇧 English](README.en.md)
- [🇯🇵 日本語](README.ja.md)
- [🇰🇷 한국어](README.ko.md)

---

## 📄 开源协议

本项目采用 **MIT + 专有许可** 的双模式许可：

- **基础功能** (MIT License)：核心语音输入、离线模型、本地词库管理等
- **增值服务** (专有许可)：用户登录、云同步、词库市场、AI 增强服务等

详见 [LICENSE.txt](LICENSE.txt)

---

## 🗺️ 产品路线

WordFlow 的未来发展规划包括：

- **阶段一** (2026 年 3 月)：基础完善，开源发布
- **阶段二** (2026 年 4-5 月)：用户系统、支付集成
- **阶段三** (2026 年 6-7 月)：词库市场上线
- **阶段四** (2026 年 8-9 月)：AI 增强服务
- **阶段五** (2026 年 10 月+)：持续优化、多平台支持

详见 [产品路线文档](产品路线/WordFlow 产品路线与商业化规划.md)

---

**WordFlow v1.0** - 让语音输入更智能、更便捷
